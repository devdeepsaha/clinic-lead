...
export default function StatDonut({ pct, stats }) {
  const donutPaths = useMemo(() => {
    ...
    const segments = [
      { count: stats.message,   color: "#9855f6" }, // Message
      { count: stats.call,      color: "#10b981" }, // Call
      { count: stats.skip,      color: "#D3D3D3" }, // Skip
    ...
    const gapAngle = 2;
    const availableAngle = 360 - segments.length * gapAngle;
        ...
        <div className="mt-6 w-full grid grid-cols-2 gap-x-4 gap-y-2 text-[11px] font-bold">
          <div className="flex items-center justify-between">
            <span className="text-slate-500 flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-sm inline-block" style={{ background: '#9855f6' }}></span>Message
            </span>
            <span className="text-slate-800">{stats.message}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-500 flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-sm inline-block" style={{ background: '#10b981' }}></span>Call
            </span>
            <span className="text-slate-800">{stats.call}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-500 flex items-center gap-1.5">
